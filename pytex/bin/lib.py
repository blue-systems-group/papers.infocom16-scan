...
def clean(inlines):
  ...
  keeptex = re.compile(r"\\(textit|textbf|texttt|textsc|sloppypar|sloppy)\{([^\}]+)\}", re.M)
  while True:
    beforelines = inlines
  ...
  netsense_macro = re.compile(r"\\NetSense{}", re.M)
  inlines = netsense_macro.sub("NetSense", inlines)
